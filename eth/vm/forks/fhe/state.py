...
from .computation import FheComputation
from .constants import REFUND_SELFDESTRUCT
from .transaction_context import (  # noqa: F401
    BaseTransactionContext,
    FheTransactionContext
)
from .validation import validate_fhe_transaction


class FheTransactionExecutor(BaseTransactionExecutor):
...
class FheState(BaseState):
    computation_class = FheComputation
    transaction_context_class = FheTransactionContext  # type: Type[BaseTransactionContext]
    account_db_class = AccountDB  # Type[BaseAccountDB]
    transaction_executor = FheTransactionExecutor  # Type[BaseTransactionExecutor]

    def validate_transaction(self, transaction):
        validate_fhe_transaction(self.account_db, transaction)
