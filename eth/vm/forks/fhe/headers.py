...
from .constants import (
    FHE_DIFFICULTY_ADJUSTMENT_CUTOFF
)


def compute_fhe_difficulty(parent_header, timestamp):
    """
    Computes the difficulty for a fhe block based on the parent block.
    """
    ...
    # minimum to allow for the initial fhe *warming* period during which
    # the difficulty begins lower than the protocol minimum.
    difficulty_minimum = min(parent_header.difficulty, DIFFICULTY_MINIMUM)

    if timestamp - parent_header.timestamp < FHE_DIFFICULTY_ADJUSTMENT_CUTOFF:
        base_difficulty = max(
