...
class FheTransaction(BaseTransaction):
    ...
    def get_intrinsic_gas(self):
        return _get_fhe_intrinsic_gas(self.data)

    def get_message_for_signing(self):
        return rlp.encode(FheUnsignedTransaction(
            nonce=self.nonce,
    ...
    def create_unsigned_transaction(cls, nonce, gas_price, gas, to, value, data):
        return FheUnsignedTransaction(nonce, gas_price, gas, to, value, data)


class FheUnsignedTransaction(BaseUnsignedTransaction):
...
def _get_fhe_intrinsic_gas(transaction_data):
